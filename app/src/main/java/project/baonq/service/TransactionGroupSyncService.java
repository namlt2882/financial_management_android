...
public class TransactionGroupSyncService implements Runnable {
    ...
    public Long getLastUpdateTime() {
        ...
        return sharedPreferences.getLong(TRANC_GROUP_LASTUPDATE, Long.parseLong("0"));
    }
    ...
    public void syncWithLocal(List<TransactionGroup> groups) {
        ...
        List<TransactionGroup> origin = groupDAO.findByServerId(groups.stream()
                .filter(group -> {
                    try {
                        String a = new ObjectMapper().writeValueAsString(group.ledgerWithoutContext());
                    ...
                    if (group.ledgerWithoutContext() != null && group.getLedger_id() == null) {
                        Ledger ledger = ledgerSyncService.findByServerId(group.ledgerWithoutContext().getServer_id());
                        if (ledger != null) {
